...
inline constexpr std::array<component_of_time_format, 32> http_time_format{
    component_of_time_format::day_name,
    component_of_time_format::comma,
    component_of_time_format::SP,
    component_of_time_format::day,
    component_of_time_format::SP,
    component_of_time_format::month_name,
    component_of_time_format::SP,
...
inline constexpr std::array<component_of_time_format, 32>
    utc_time_without_punctuation_format{component_of_time_format::year,
        component_of_time_format::month,
        component_of_time_format::day,
        component_of_time_format::T,
        component_of_time_format::hour,
...
{
    return week_table[((str[0] & ~0x20) ^ (str[2] & ~0x20))
        % week_table.size()];
}
...
inline std::pair<bool, std::time_t> faster_mktime(int year,
    int month,
    int day,
    int hour,
    int min,
    ...
    int day_of_week)
{
    ...
    return {
        true, std::int64_t(abs) + (absolute_to_internal + internal_to_unix)};
}
...
inline std::pair<bool, std::time_t> get_timestamp(const String& gmt_time_str)
{
    ...
    for (auto& comp : real_format)
    {
        ...
        case component_of_time_format::year:
            ...
            if ((year = get_digit(sv.substr(len_of_processed_part, 4), 4))
                == -1)
            {
                return {false, 0};
            }
        ...
        case component_of_time_format::day:
            ...
            int digit;
            if ((digit = get_digit(sv.substr(len_of_processed_part, 2), 2))
                == -1)
            {
                return {false, 0};
            }
        ...
        case component_of_time_format::day_name:
            ...
            if ((day_of_week
                    = get_day_index(sv.substr(len_of_processed_part, 3)))
                < 0)
            {
                return {false, 0};
            }
        ...
        case component_of_time_format::second_decimal_part:
            ...
            while (
                cur < len_of_gmt_time_str && (sv[cur] >= '0' && sv[cur] <= '9'))
            {
                len_of_ignored_part++;
                cur++;
            }
...
constexpr std::string_view WDAY[7]
    = {"Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"};
constexpr std::string_view YMON[12] = {"Jan",
    "Feb",
    "Mar",
    "Apr",
    "May",
...
inline std::string_view get_local_time_str(char (&buf)[N],
    std::time_t t,
