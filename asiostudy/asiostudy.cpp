...
protected:
    // Construct the actor to use the specified executor for all message
    // handlers.
    ...
    void deregister_handler(void (Actor::*mf)(Message, actor_address))
    {
        ...
        for (auto iter = handlers_.begin(); iter != handlers_.end(); ++iter)
        {
            ...
            {
                auto mh = static_cast<mf_message_handler<Actor, Message>*>(
                    iter->get());
...
int actortest()
{
    ...
    // Initialise the actors by passing each one the address of the next actor
    // in the ring.
    for (std::size_t i = num_actors, next_i = 0; i > 0; next_i = --i)
        send(members[next_i]->address(),
            rcvr.address(),
            members[i - 1]->address());
    ...
    // Wait for all signal messages, indicating the tokens have all reached
    // zero.
    for (std::size_t i = 0; i < num_actors; ++i)
        rcvr.wait();
    return 0;
...
void postdispatchtest()
{
    ...
    std::thread t([&io_context, &s]() {
        ...
        {
            // post到t2中执行
            asio::post(io_context, [&io_context]() {
                // 继续post回调函数
                asio::post(io_context, []() {
                    cout << "post will run after all dispatch, threadid:"
                         << std::this_thread::get_id() << "\n";
                });
                // dispatch方法和run方法在同一线程，直接执行回调函数
                asio::dispatch(io_context, []() {
                    cout << "dispatch1 will run first, threadid:"
                         << std::this_thread::get_id() << "\n";
                });
                asio::dispatch(io_context, []() {
                    cout << "dispatch2 will run first, threadid:"
                         << std::this_thread::get_id() << "\n";
                });
            });
...
{
    asio::io_context
        io;  // 无论是post还是dispatch，其待调用的函数都是在io_context::run的线程里面运行
    asio::strand<asio::io_context::executor_type> str(
        io.get_executor());  // strand的用法，这里用来防止cout输出错乱

    thread t1([&io, &str]() {
        (void)str;  // unused warning
        // asio::executor_work_guard<asio::io_context::executor_type> wg
        auto wg = asio::make_work_guard(
            io);  // executor_work_guard可以防止io.run()在事件处理完以后退出
    ...
    thread t2([&io, &str]() {
        ...
        for (int i = 0; i < 10; ++i)
        {
            ...
            asio::dispatch(io, asio::bind_executor(str, []() {
                cout << "dispatch, threadid:" << std::this_thread::get_id()
                     << "\n";
            }));
            asio::post(io, asio::bind_executor(str, []() {
            ...
            asio::post(io, [&io, &str]() {
                asio::dispatch(io, asio::bind_executor(str, []() {
                    cout << "dispatch will run first, threadid:"
                         << std::this_thread::get_id() << "\n";
                }));
                asio::post(io, asio::bind_executor(str, []() {
                    cout << "post will run after all dispatch, threadid:"
...
void strandtest()
{
    ...
    for (int i = 0; i < 3; i++)
        runthreads.emplace_back(std::thread([&io_context]() {
            io_context.run();
        }));
    auto fun1 = []() {
    ...
    {
        // 顺序绑定strand
        asio::strand<asio::io_context::executor_type> fun1strand(
            asio::make_strand(io_context));
        asio::strand<asio::io_context::executor_type> fun2strand(
            asio::make_strand(io_context));
