...
#include "meta_string.hpp"
...
#if defined(_WIN32)
    #ifndef _WINDOWS_
        #ifndef WIN32_LEAN_AND_MEAN  // Sorry for the inconvenience. Please
                                     // include any related headers you need
                                     // manually.
        ...
        #undef ERROR  // Windows GDI defines this macro; make it a global enum
                      // so it doesn't conflict with our code
enum
{
    ERROR = 0
};
...
public:
    ...
    std::string_view get_file_str() const
    {
        return file_str_;
    }
    ...
    record_t& operator<<(const T& data)
    {
        ...
        else if constexpr (std::is_integral_v<U>)
        {
            ...
            && !std::is_same_v<const char*, U> && !std::is_same_v<char*, U>)
        {
            char buf[32] = {"0x"};
            auto [ptr, ec]
                = std::to_chars(buf + 2, buf + 32, (uintptr_t)data, 16);
            ss_.append(buf, std::distance(buf, ptr));
        }
