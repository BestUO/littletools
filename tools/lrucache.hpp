...
public:
    ...
    LRUCache(size_t max_size = 100, size_t remain_days = 1)
        : __max_size(max_size)
    ...
    void StoreKeyValue(const K& key, const V& value)
    {
        ...
        while (__cache_items_map.size() > __max_size)
        {
            ...
            last--;
            if (std::chrono::system_clock::now() - last->second.create_time
                > std::chrono::hours(24 * __remain_days))
            {
    ...
    std::optional<V> GetValue(const K& key)
    {
        ...
        else
        {
                ...
                > std::chrono::hours(24))
            {
                it->second->second.create_time
                    = std::chrono::system_clock::now();
                __cache_items_list.splice(
                    __cache_items_list.begin(), __cache_items_list, it->second);
            }
