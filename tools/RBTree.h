...
extern void rb_replace_node(const struct rb_node* victim,
    ...
    struct rb_node** link)
{
    ...
    node->rb_left = node->rb_right = static_cast<struct rb_node*>(0);

    *link = node;
}

template <class P, class M>
inline constexpr std::size_t offsetOf(const M P::*member)
{
    return reinterpret_cast<std::size_t>(&(reinterpret_cast<P*>(0)->*member));
}

template <class P, class M>
inline constexpr P* container_of_impl(M* ptr, const M P::*member)
{
    return reinterpret_cast<P*>(
        reinterpret_cast<char*>(ptr) - offsetOf(member));
}
