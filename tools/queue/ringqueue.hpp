...
public:
    ...
    std::optional<
        typename std::enable_if<std::is_same<C<T, A>, std::vector<T>>::value,
            C<T, A>>::type>
    ...
    std::optional<typename std::
            enable_if<std::is_same<C<T>, std::queue<T>>::value, C<T>>::type>
    GetObjBulk(unsigned int n)
    {
...
private:
    ...
    std::tuple<bool, unsigned int, unsigned int> MoveProdHead(size_t n)
    {
        ...
        while (success == 0)
        {
            ...
            auto free_entries
                = ringinfo.capacity + ringinfo.cons.tail.load() - old_head;
            if (n > free_entries)
                return std::tuple<bool, unsigned int, unsigned int>(
                    false, 0, 0);
            ...
            if (ringinfo.single)
            {
                ...
                success = ringinfo.prod.head.compare_exchange_weak(
                    old_head, new_head);
        }
        return std::tuple<bool, unsigned int, unsigned int>{
            true, old_head, new_head};
    }
    ...
    std::tuple<bool, unsigned int, unsigned int> MoveConsHead(unsigned int& n)
    {
        ...
        while (success == 0)
        {
            ...
            if (ringinfo.single)
            {
                ...
                success = ringinfo.cons.head.compare_exchange_weak(
                    old_head, new_head);
        }
        return std::tuple<bool, unsigned int, unsigned int>{
            true, old_head, new_head};
    ...
    typename std::enable_if<
        is_specialization_of<typename std::decay<C>::type, std::vector>::value,
        C>::type
    ...
    template <class C>
    typename std::enable_if<
        is_specialization_of<typename std::decay<C>::type, std::queue>::value,
        C>::type
    GetItems(unsigned int old_head, unsigned int n)
    {
        return GetItems<C>(
    ...
    void UpdateTail(RingHeadTail& ht,
        unsigned int old_val,
        unsigned int new_val)
    {
        ///////error////////////////
        // while(!ht.tail.compare_exchange_weak(old_val,
        // new_val,std::memory_order_consume))
        // {}
        ///////error////////////////
