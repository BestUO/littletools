...
{
    auto tmp = root.lock();
    auto [node_type,node_txt,prompt_txt, keywords] = DBOperate::GetInstance()->GetNodeTypeAnswerPromptTxt(course_id, tmp->node_code)[0];
    if(node_type==1)
    {
        ...
        questiondetail->keywords = std::move(keywords);
