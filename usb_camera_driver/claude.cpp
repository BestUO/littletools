...
private:
    ...
    // 输出流相关
    bool createOutputContext(const std::string& filename, AVFormatContext** out_ctx, AVStream** out_stream);
...
public:
    USBCamera(const std::string& device = "/dev/video0", 
              int w = 640, int h = 480, int framerate = 30,
    ...
    int getFrameRate() const { return fps; }
...
USBCamera::USBCamera(const std::string& device, int w, int h, int framerate, CameraFormat fmt)
    : format_ctx(nullptr), decode_ctx(nullptr), decoder(nullptr),
      encode_ctx(nullptr), encoder(nullptr),
      frame(nullptr), frame_rgb(nullptr), frame_yuv(nullptr), packet(nullptr),
      sws_ctx_rgb(nullptr), sws_ctx_yuv(nullptr),
      video_stream_idx(-1), device_path(device), 
      width(w), height(h), fps(framerate), format(fmt), initialized(false) {
    ...
    std::cout << "  Format: " << (format == CameraFormat::MJPEG ? "MJPEG" : 
                                 format == CameraFormat::H264 ? "H264" : "YUV422") << std::endl;
}
...
bool USBCamera::initializeFFmpeg() {
    ...
    std::cout << "使用输入格式: " << input_format->name 
...
bool USBCamera::initializeDecoder() {
    ...
    std::cout << "解码器初始化成功 - 实际分辨率: " << width << "x" << height << std::endl;
    return true;
}
...
bool USBCamera::initializeFramesAndScaler() {
    ...
    // 为RGB帧分配缓冲区
    int num_bytes_rgb = av_image_get_buffer_size(AV_PIX_FMT_RGB24, width, height, 1);
    ...
    // 创建缩放上下文 - 解码后转RGB
    sws_ctx_rgb = sws_getContext(width, height, decode_ctx->pix_fmt,
    ...
    // 创建缩放上下文 - RGB转YUV用于编码
    sws_ctx_yuv = sws_getContext(width, height, AV_PIX_FMT_RGB24,
...
bool USBCamera::captureFrame() {
    ...
    if(format == CameraFormat::MJPEG) {
        std::string filename = "frame_" + std::to_string(av_gettime_relative()) + ".jpg";
    ...
    // 转换像素格式为RGB
    sws_scale(sws_ctx_rgb, (uint8_t const * const *)frame->data, frame->linesize, 
...
bool USBCamera::createOutputContext(const std::string& filename, AVFormatContext** out_ctx, AVStream** out_stream) {
    // 创建输出格式上下文
    if (avformat_alloc_output_context2(out_ctx, nullptr, "mjpeg", filename.c_str()) < 0) {
    ...
    *out_stream = avformat_new_stream(*out_ctx, nullptr);
    ...
    // 复制编码器参数到输出流
    if (avcodec_parameters_from_context((*out_stream)->codecpar, encode_ctx) < 0) {
...
bool USBCamera::saveFrameAsJPG(const std::string& filename) {
    ...
    sws_scale(sws_ctx_yuv, frame_rgb->data, frame_rgb->linesize,
...
//     // 转换RGB到YUV
//     sws_scale(sws_ctx_yuv, (uint8_t const * const *)frame_rgb->data, frame_rgb->linesize,
//               0, height, frame_yuv->data, frame_yuv->linesize);
