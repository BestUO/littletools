...
#include "iot/http/request_handler/system_mgr_handler.h"
#include "jsoncpp/json/json.h"
#include "lim/transport/http/http_request_session.h"

namespace iot::http {
    SystemMgrHandler::SystemMgrHandler(std::shared_ptr<rclcpp::Node> node) : RequestHandler(node) {
        //设置支持处理的url集合
    ...
    SystemMgrHandler::~SystemMgrHandler() = default;

    bool SystemMgrHandler::HandleRequest(BytesMessage &request, RequestDispatcher *dispatcher) {
        ...
        auto system_client             = node_->create_client<app_interfaces::srv::IotServerInfo>(service_name);

        //调用service
        if (!system_client->wait_for_service(std::chrono::seconds(5))) {
        ...
        auto future_result = system_client->async_send_request(srv_request);
        ...
        dispatcher->SendResponse(response->result);
        return true;
    }
};// namespace iot::http
