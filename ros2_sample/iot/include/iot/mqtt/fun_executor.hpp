...
class FunExecutor
{
   public:
    ...
    void RegisterFunction(const std::string &name, Function &&func)
    {
        functions_[name] = [func = std::forward<Function>(func)](TupleWrapper params, void *result) mutable
        {
            ...
            {
                InvokeFromAny<args_tuple>(func, std::move(params.GetParams()), std::make_index_sequence<std::tuple_size_v<args_tuple>>{});
            }
            else
            {
                if (result == nullptr)
   ...
   private:
    ...
    {
        return std::invoke(std::forward<F>(func), AnyToDestType<std::tuple_element_t<Indices, Args_tuple>>(args[Indices])...);
    }
};
}  // namespace v1
...
{
   public:
    void RegisterFunction(const std::string &name, std::function<nlohmann::json(nlohmann::json &&)> &&func)
    {
        functions_[name] = std::forward<decltype(func)>(func);
    }

    nlohmann::json InvokeFunction(const std::string &name, nlohmann::json &&args)
    {
        auto it = functions_.find(name);
        if (it != functions_.end())
        {
   ...
   private:
    std::map<std::string, std::function<nlohmann::json(nlohmann::json &&)>> functions_;
};
}  // namespace v2
}  // namespace iot::mqtt