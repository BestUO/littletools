...
static inline std::vector<uint8_t> build_avcc_extradata_from_sps_pps(const std::vector<uint8_t> &sps, const std::vector<uint8_t> &pps)
{
...
{
   public:
    StreamH264Pusher() : format_ctx_(nullptr), video_stream_(nullptr), codec_ctx_(nullptr), is_running_(false), frame_count_(0)
    {
    }
    ...
    void SetCodecHeaderAnnexB(const std::vector<uint8_t> &header)
    {
        ...
        {
            uint32_t len = (uint32_t)header[off] << 24 | (uint32_t)header[off + 1] << 16 | (uint32_t)header[off + 2] << 8 | (uint32_t)header[off + 3];
            off += 4;
            ...
            codec_header_annexb_.insert(codec_header_annexb_.end(), header.begin() + (long)off, header.begin() + (long)(off + len));
            off += len;
        }
    ...
    bool Start()
    {
        ...
        printf("%s pusher initialized successfully: %s\n", protocol_ == StreamProtocol::RTSP ? "RTSP" : "RTMP", url_.c_str());
        return true;
    }
        ...
        if (protocol_ == StreamProtocol::RTMP && !header_written_)
        {
            ...
            if (sps.empty() || pps.empty())
            {
                auto try_parse_avcc = [&](const uint8_t *p, size_t n)
                {
                    ...
                    while (off + 4 <= n)
                    {
                        uint32_t len = (uint32_t)p[off] << 24 | (uint32_t)p[off + 1] << 16 | (uint32_t)p[off + 2] << 8 | (uint32_t)p[off + 3];
                        off += 4;
            ...
            if (!sps.empty() && !pps.empty())
            {
                ...
                if (!extra.empty())
                {
                    ...
                    video_stream_->codecpar->extradata      = (uint8_t *)av_mallocz(extra.size() + AV_INPUT_BUFFER_PADDING_SIZE);
    ...
    void Stop()
    {
        ...
        printf("%s pusher stopped. Total frames: %ld\n", protocol_ == StreamProtocol::RTSP ? "RTSP" : "RTMP", frame_count_.load());
    }
