...
namespace ormpp {
    ...
    constexpr std::array<size_t, sizeof...(members)> indexs_of() {
        return std::array<size_t, sizeof...(members)>{ylt::reflection::index_of<members>()...};
    }
...
#define REGISTER_AUTO_KEY(STRUCT_NAME, KEY)                                                                            \
    inline auto ORMPP_UNIQUE_VARIABLE(STRUCT_NAME) = ormpp::add_auto_key_field(#STRUCT_NAME, #KEY);

#define REGISTER_AUTO_KEY_WITH_NAMESPACE(STRUCT_NAME, KEY)                                                             \
    inline auto ORMPP_UNIQUE_VARIABLE(STRUCT_NAME) =                                                                   \
            ormpp::add_auto_key_field(ylt::reflection::type_string<STRUCT_NAME>(), #KEY);
    ...
    inline int add_conflict_key_field(std::string_view key, std::string_view value) {
...
#define REGISTER_CONFLICT_KEY_WITH_NAMESPACE(STRUCT_NAME, ...)                                                         \
    inline auto ORMPP_UNIQUE_VARIABLE(STRUCT_NAME) =                                                                   \
            ormpp::add_conflict_key_field(ylt::reflection::type_string<STRUCT_NAME>(), {MAKE_NAMES(__VA_ARGS__)});
    ...
    template<typename T>
    constexpr std::enable_if_t<iguana::non_ylt_refletable_v<T>, size_t> get_value() {
        return 1;
    }
    ...
    inline constexpr auto get_type_names(DBType type) {
        ...
        ylt::reflection::for_each(T{}, [&arr, type](auto &field, auto /*name*/, auto index) {
...
#ifdef ORMPP_ENABLE_PG
        std::string quota_name = "\"" + std::string(ylt::reflection::get_struct_name<T>()) + "\"";
#else
        std::string quota_name = "`" + std::string(ylt::reflection::get_struct_name<T>()) + "`";
#endif
        return quota_name;
    ...
    inline std::string generate_insert_sql(bool insert, Args &&...args) {
        ...
        for (size_t i = 0; i < Count; ++i) {
            std::string field_name(ylt::reflection::name_of<T>(i));
            if (insert && is_auto_key<T>(field_name)) { continue; }
#ifdef ORMPP_ENABLE_PG
        ...
        if (fields.back() != ')') { fields.back() = ')'; }
        if (values.back() != ')') { values.back() = ')'; }
...
#ifdef ORMPP_ENABLE_PG
            (fields.append(ylt::reflection::name_of<T>(ylt::reflection::index_of<members>()))
                     .append("=$" + std::to_string(++index) + ","),
             ...);
#else
            (fields.append(ylt::reflection::name_of<T>(ylt::reflection::index_of<members>())).append("=?,"), ...);
#endif
        } else {
            constexpr auto Count = ylt::reflection::members_count_v<T>;
            for (size_t i = 0; i < Count; ++i) {
    ...
    inline std::string generate_query_sql(Args &&...args) {
        ...
        if (where) { append(sql, "where 1=1 and "); }
...
#define FID(field)                                                                                                     \
    std::pair<std::string_view, decltype(&field)>(ormpp::get_field_name<decltype(&field)>(std::string_view(#field)),   \
                                                  &field)
