...
auto UpdateOP(dbng<sqlite> &db, std::vector<person> &persons) {
    if (!persons.empty()) {
        ...
        std::cout << "Updated partial fields with id in query response like: update `person` set age=99 where id="
                  << first_person.id << std::endl;
        db.update_some<&person::age>(first_person);
        PrintAllInDB();

        std::cout << "\n--- Updated all fields with id in query response like: update `person` set id="
                  << first_person.id << ",name=test4,age=100 where id=" << first_person.id << "---" << std::endl;
        first_person.age = 100;
        db.update(first_person);
        PrintAllInDB();

        std::cout << "\n--- Updated with custom condition like: update `person` set age=12 where name='test4' ---"
                  << std::endl;
        person tmp = {0, "", 12};
...
int main() {
    ...
    try {
        ...
        SetWal(db);                //set WAL mode for better performance
        InsertOP(db);              //insert operations
        auto persons = QueryOP(db);//query operations
        UpdateOP(db, persons);     //update operations
        DeleteOP(db);              //delete operations
        TransactionOP(db);         //transaction operations
        RawSql(db);                //raw SQL execution example
