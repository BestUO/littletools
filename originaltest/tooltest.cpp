...
int ttt()
{
    ...
    server.set_http_handler<GET, POST>("/", [&](request& req, response& res) {
        ...
        SimpleWrapKafka::inst().Add2Kafka(
            "mytest", ip + uuid + "_testfromcinatra");

        // std::string
        // filename("/home/uncle_orange/BorayProduct/tools/www/1616579920094476511.jpg");
        // auto file = std::make_shared<std::ifstream>(filename,
        // std::ios::binary); std::string content; const size_t size = 5 * 1024
        // * 1024; content.resize(size); file->read(&content[0], size); int64_t
        // read_len = (int64_t)file->gcount();

        // if (read_len < size)
        //     content.resize(read_len);
        // res.set_status_and_content(status_type::ok,
        // std::move(content),req_content_type::multipart);
...
int cinatra_uploadfile()
{
    ...
    server.set_http_handler<GET, POST>(
        "/upload_multipart", [](request& req, response& res) {
            ...
            res.set_delay(true);
            // res.set_status_and_content(status_type::ok, "multipart
            // finished");
...
void testKafkaCinatra()
{
    ...
    SimpleWrapKafka::inst().CreateConsumer(
        brokers, topic, groupid, [](std::string message) {
...
void testtimermanager()
{
    ...
    timermanager::v1::TimerManager<TTT>* a
        = timermanager::v1::TimerManager<TTT>::GetInstance();
    sleep(10);

    a->AddAlarm(std::chrono::seconds(20),
        TTT{"run fun1", 20},
        std::bind(fun1, TTT{"run fun1", 10}));
    a->AddAlarm(std::chrono::seconds(5),
        TTT{"run fun1", 3},
        std::bind(fun1, TTT{"run fun1", 3}));
    a->AddAlarm(std::chrono::seconds(5),
        TTT{"run fun1interval", 3},
        std::bind(fun1, TTT{"run fun1interval", 3}),
        std::chrono::seconds(1));

    for (int i = 0; i < 5; i++)
        a->AddAlarm(std::chrono::seconds(10),
            TTT{"run fun1", i},
            std::bind(fun1, TTT{"run fun1", i}));

    // a->DeleteAlarm(std::bind([](const TTT& t, int id){ return t.id == id;
    // },std::placeholders::_1, 3));

    auto fun2 = [](int a, int b) {
        std::cout << a << '\t' << b << std::endl;
    };
    timermanager::v1::TimerManager<TTT>* b
        = timermanager::v1::TimerManager<TTT>::GetInstance();
    for (int i = 5; i < 10; i++)
        b->AddAlarm(
            std::chrono::seconds(2), TTT{"123", i}, std::bind(fun2, i, i + 1));
    b->AddAlarm(std::chrono::seconds(2), TTT{"123", 5}, std::bind(fun2, 5, 6));
    b->AddAlarm(std::chrono::seconds(2), TTT{"123", 6}, std::bind(fun2, 6, 7));
}
...
void testregister()
{
    ...
    FunctionsManager::GetInstance()->register_handler(
        "test2", [](std::string a) {
            std::cout << a << std::endl;
        });
    // auto r = FunctionsManager::GetInstance()->call<std::string>("test",
    // TTT()); std::cout << r << std::endl;
    // FunctionsManager::GetInstance()->call<std::string>("test2",
    // std::string("ss"));
...
void testRingFreeLockQueue()
{
    ...
    int vc3[100000] = {0};
    bool run        = true;
    std::cout << "numeric_limits<unsigned int>::max()= "
    ...
    auto c = [&run, &q](int* vc, int* n) {
        while (run)
        {
            ...
            if (e)
            {
                // std::cout << std::this_thread::get_id() << " cnum:"<< e <<
                // std::endl;
                vc[*e]++;
                (*n)++;
            }
...
{
    using QueueType = std::conditional_t<true,
        LockQueue<std::function<void()>>,
        FreeLockRingQueue<std::function<void()>>>;
    threadpool::v1::ThreadPool<QueueType> pool(2);
    std::cout << "atart" << std::endl;
    std::vector<std::future<int>> results;
    for (int i = 0; i < 3; ++i)
    {
        results.emplace_back(pool.EnqueueFun([i]() {
...
struct Worker2Params
{
    ...
    Worker2Params(int a)
        : worker2paramsa(a){};
    Worker2Params(Worker2Params&& other)
        : worker2paramsa(other.worker2paramsa){};
    Worker2Params(const Worker2Params& other)
        : worker2paramsa(other.worker2paramsa){};
...
template <class T>
class Worker2 : public threadpool::v1::Worker<T>
{
public:
    Worker2(std::shared_ptr<T> queue)
...
protected:
    ...
    virtual typename std::enable_if<
        std::is_same<typename T::Type, Worker2Params>::value>::type
    // typename std::enable_if<std::is_same<typename GetContainerType<T>::Type,
    // Worker2Params>::value>::type
...
void testPThreadPool()
{
    using QueueType = std::conditional_t<false,
        LockQueue<Worker2Params>,
        FreeLockRingQueue<Worker2Params>>;
    auto queuetask  = std::shared_ptr<QueueType>(new QueueType);
    std::shared_ptr<threadpool::v1::Worker<QueueType>> worker
        = std::make_shared<Worker2<QueueType>>(queuetask);

    threadpool::v1::ThreadPool pool(queuetask, worker, 2);
    for (int i = 0; i < 10; i++)
...
REFLECTION(aicall_tts_file_cache,
    id,
    TTS_text,
    TTS_version_code,
    tts_src,
...
void testormpp()
{
    ...
    for (auto& file : res)
    {
        std::cout << file.id << " " << file.TTS_text << " "
                  << file.TTS_version_code << std::endl;
    }
}
...
void testrapidjson()
{
    ...
    const char* json
        = "{\"mysql_setting\":{\"mysql_user\":\"user\",\"mysql_password\":"
          "\"123\",\"mysql_db\":\"db\",\"mysql_host\":\"127.0.0.1\"}}";
...
template <class T>
class Worker3 : public threadpool::v1::Worker<T>
{
public:
    Worker3(std::shared_ptr<T> queue)
        : threadpool::v1::Worker<T>(queue){};
...
protected:
    ...
    virtual typename std::enable_if<
        std::is_same<typename T::Type, int>::value>::type
    DealElement(int&& i)
    {
        sum += i;
    }
...
void testQueueTypeThreadPool()
{
    ...
    using QueueType
        = std::conditional_t<false, LockQueue<int>, FreeLockRingQueue<int>>;
    auto queuetask = std::shared_ptr<QueueType>(new QueueType);
    std::shared_ptr<threadpool::v1::Worker<QueueType>> worker
        = std::make_shared<Worker3<QueueType>>(queuetask);

    threadpool::v1::ThreadPool pool(queuetask, worker, 1, 1);
