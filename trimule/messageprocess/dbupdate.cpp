...
void UpdateMessage::CheckAndUpdateAicallCalllogContinuousSync(CallInfo &calllog, const std::string &id, ormpp::dbng<ormpp::mysql> &mysqlclient)
{
	...
	std::vector<std::string> columns = {"call_record_url", "intention_type", "transfer_manual_cost",
	...
	std::vector<std::string> values = {calllog.record_url, intention_type, std::to_string(calllog.transfer_manual_cost), std::to_string(calllog.call_state), 
										calllog.switch_number, std::to_string(calllog.ring_time), std::to_string(calllog.hangup_type), calllog.transfer_start_time, calllog.transfer_confirm_time, 
...
{
	std::vector<std::string> columns = {"duration","call_time", "transfer_number", "transfer_duration", "call_record_url", "manual_status", "answer_time", "hangup_time"};
	std::string manual_status = calllog.manual_type == 0 ? "" : std::to_string(calllog.manual_type);
	std::vector<std::string> values = {std::to_string(calllog.duration_time),calllog.start_time, calllog.transfer_number, std::to_string(calllog.transfer_duration), calllog.record_url, manual_status, calllog.confirm_time, calllog.end_time};
	std::vector<std::string> condition(1);
	condition[0] = calllog.cc_number;
	std::vector<std::string> condition_name(1);
...
{
	std::vector<std::string> columns = {"manual_status", "call_time", "call_duration"};

	std::string manual_status = calllog.manual_type == 0 ? "" : std::to_string(calllog.manual_type);
	std::vector<std::string> values = {manual_status, calllog.start_time, std::to_string(calllog.duration_time)};
	std::vector<std::string> condition(1);
	condition[0] = clue_id;
...
void UpdateMessage::UpdateAiCalllogExtension(CallInfo &calllog, std::string &calllog_id, ormpp::dbng<ormpp::mysql> &mysqlclient)
{
	...
	std::vector<std::string> values = {std::to_string(calllog.transfer_manual_cost), std::to_string(calllog.ring_time), call_state, switch_number, hangup_cause_, calllog.transfer_start_time, calllog.transfer_confirm_time, calllog.transfer_end_time};
	std::vector<std::string> condition(1);
